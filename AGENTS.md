...
6. Charts — sub-character gradient fill via eighth-block Unicode glyphs, percent-driven green/amber/red coloring instead of one flat color.
...
- **`renderReports`'s heatmap-fit guard undercounted lines**: `remaining := height - len(sections)` counted string _elements_, not rendered _lines_ — several `sections` entries are multi-line strings (e.g. each progress row is 2 lines), so the guard overestimated available headroom before deciding whether to render the heatmap. Fixed by counting actual `\n`-delimited lines per section.
...
2. **`renderPomodoro`'s phase line** (`internal/ui/pomodoro.go`) — `fmt.Sprintf("%s  %s", phaseStyle.Render(...), statLabelStyle.Render(...))` put two raw spaces _between_ two independently-styled segments, outside both SGR spans. Fixed by folding the separator into the second segment's own `Render` call.
...
- The typed branch pads to `m.Width` and then appends a one-cell cursor block _past_ that padding, so its real width is `Width + 1`.
...
Applied consistently to all four titled panes. Reports and Pomodoro previously drew their own title _inside_ the body (they passed `title=""`); those rows were removed and the titles passed to `renderPane` instead, otherwise the app would have had two different title treatments. Freed body rows were returned to content:
...
**6 pips = the whole cycle.** The user asked for 25→5→25→5→25→15, which is a change to the CYCLE, not just the display: `longBreakEvery` went 4 → 3, and the pips now represent every _phase_ of a cycle (3 work + 3 breaks) rather than just work sessions, so breaks are visible too. `cyclePosition()` maps (completed, phase) onto [0, 6).
...
**Regression caught and fixed (second time for this same mechanism).** Adding the status line took Pomodoro to 10 content lines and pushed Reports' heatmap from 40 rows back up to 45 in layout 1. Root cause was `pomoMinContentLines` being _derived_ as `pomoContentLines - 3`, so every line added to Pomodoro silently raised its floor too, consuming slack Reports needs. Now stated as an explicit `= 6` (header, 3 digit rows, status, dots) with a comment explaining why it must not be derived. `renderPomodoro` gained a second degradation step to reach it: blank spacers first, then the **progress bar** — the big countdown already conveys progress, whereas phase/timer/status/dots each carry information nothing else shows.
...
**Layout.** Under Pomodoro in the column layouts; a third task-row column in the stacked layout (Today/Overdue/Notes each a third, Notes absorbing the rounding remainder). Unlike Pomodoro, Notes does _not_ fully yield to Reports — a requested section that only appears on very tall terminals isn't much of a section — so it takes a fixed share capped at a quarter of the info column, and Reports absorbs the cost. That's the right trade because `renderReports` already degrades gracefully (dropping heatmap, then streak), whereas a 1-row Notes pane is simply unusable; below `notesMinContentLines` it's omitted entirely rather than rendered too small.
...
## Use XDG base directory for user data & configuration (2026-09-07)

Fixed [#6](https://github.com/parsaenami/taskii/issues/6): relative path dependency on `$PWD` causing missing data when running taskii from different directories.

- Integrated `github.com/adrg/xdg` library for cross-platform standard XDG paths.
- Data files (`tasks.json`, `notes.json`) now resolve to `xdg.DataFile("taskii/...")` (`$XDG_DATA_HOME/taskii/` or `%LOCALAPPDATA%\taskii\`).
- Configuration files (`settings.json`) now resolve to `xdg.ConfigFile("taskii/settings.json")` (`$XDG_CONFIG_HOME/taskii/` or `%APPDATA%\taskii\`).
- Verified with unit tests (`internal/model/store_test.go`) isolating the test environment with `t.Setenv` and `xdg.Reload()`.
